...
namespace {
// Die \emph{globale} Funktion \tt{ggt()} wird zum Kürzen benötigt.
// Sie
// berechnet den größten
...
const Rational add(const Rational& a, const Rational& b) {
  ...
  // Ergebnis\label{Kurzformnutzung}
  //  zurückgegeben wird.
  Rational r = a;
  //  Das temporäre Objekt \tt{r} wird benötigt, weil \tt{a} wegen
  //  der
  //  \tt{const}-Eigenschaft nicht
  //  verändert werden kann (und darf). Die eigentlich bessere
  //  Alternative wäre die \smash{Ü}bergabe
  //  per Wert -- dann könnte mit der lokalen Kopie gearbeitet
  //  werden
  //  (Begründung siehe
  //  nachfolgender Text).
  r.add(b);
